          ...
          role === "Company" ? (
            <div className="flex items-center gap-2">
              <Link
                ...
                className="bg-white hover:bg-slate-50 text-slate-755 font-bold py-2 px-4 rounded-xl border border-slate-200 transition flex items-center gap-2 text-xs shadow-sm"
              >
      ...
      <div className="space-y-6">
                    ...
                    {job.job_skills && (
                      <div className="flex items-center gap-1.5 flex-wrap pt-1">
                        ...
                        {job.job_skills.split(",").map((skill, idx) => (
                          <span key={idx} className="px-2 py-0.5 rounded-lg text-xxs font-bold bg-slate-50 text-slate-655 border border-slate-200">
                            {skill.trim()}
                          </span>
                        ))}
                      </div>
                      ...
                      <>
                        ...
                        {job.has_multiple_rounds && (
                          <Link to={`/dashboard/company/round-management/${job._id}`}
                            className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-xs font-bold bg-purple-600 text-white hover:bg-purple-705 transition shadow-sm">
                            <FiLayers className="w-3.5 h-3.5" /> Manage Rounds
                          </Link>
                        )}
                        <button onClick={() => handleDelete(job._id)}
                          className="bg-rose-50 hover:bg-rose-100 text-rose-750 font-bold py-2.5 px-3.5 rounded-xl border border-rose-200 transition text-xs flex items-center gap-1.5 shadow-sm">
        ...
        {jobs.length === 0 && (
          <div className="bg-white rounded-2xl border border-slate-200/80 p-16 text-center text-slate-400 shadow-sm flex flex-col items-center justify-center">
            <FiBriefcase className="w-12 h-12 text-slate-355 mb-3" />
