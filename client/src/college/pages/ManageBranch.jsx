...
const ManageBranch = () => {
  ...
  const fetchData = async () => {
    try {
      // 1. Fetch degrees to show in dropdown
      const degreeRes = await customFetch.get("/degree");
      const filteredDegrees = (degreeRes.data.degrees || []).filter((d) => {
        const degCollegeId = d.college_id?._id || d.college_id;
        return degCollegeId && user?._id && degCollegeId.toString() === user?._id.toString();
      });
      setDegrees(filteredDegrees);

      // 2. Fetch branches and filter by college
      const branchRes = await customFetch.get("/branch");
      const filteredBranches = (branchRes.data.branches || []).filter((b) => {
        const branchCollegeId = b.college_id?._id || b.college_id;
        return branchCollegeId && user?._id && branchCollegeId.toString() === user?._id.toString();
      });
