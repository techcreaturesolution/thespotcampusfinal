...
const DashboardLayout = () => {
  ...
  const getMenuItems = () => {
    ...
    switch (role) {
      ...
      case "University":
        ...
        { path: "/dashboard/university/manage-degree", icon: <FiBookOpen />, label: "Degrees" },
        { path: "/dashboard/university/manage-branch", icon: <FiCpu />, label: "Branches" },
        ];
      case "TPO":
        return [...base,
        { path: "/dashboard/tpo/manage-job", icon: <FiBriefcase />, label: "Jobs" },
        { path: "/dashboard/tpo/manage-student", icon: <FiUsers />, label: "Students" },
        { path: "/dashboard/tpo/manage-degree", icon: <FiBookOpen />, label: "Degrees" },
        { path: "/dashboard/tpo/manage-branch", icon: <FiCpu />, label: "Branches" },
        ];
      default:
        return base;
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      <aside
        className={`fixed inset-y-0 left-0 z-50 bg-white border-r border-gray-200 transform transition-all duration-300 lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} ${isCollapsed ? "lg:w-16 w-56" : "w-56"}`}
      >
        <div className={`flex items-center ${isCollapsed ? "lg:flex-col lg:justify-center lg:gap-2 lg:py-4 lg:px-0 lg:h-auto px-4 h-16 justify-between" : "px-5 h-16 justify-between"} border-b border-gray-100`}>
          <div className="w-8 h-8 bg-gradient-to-br from-primary-600 to-indigo-600 rounded-lg flex items-center justify-center flex-shrink-0">
            <span className="text-white font-bold text-sm">S</span>
          </div>
          <span className={`font-bold text-gray-900 transition-all duration-300 whitespace-nowrap ${isCollapsed ? "lg:w-0 lg:opacity-0 lg:hidden block" : "w-auto opacity-100 block"}`}>
            The Spot Campus
          </span>
          <button 
            type="button" 
            onClick={() => setIsCollapsed(!isCollapsed)}
            className={`hidden lg:flex p-1.5 hover:bg-gray-100 rounded-lg text-gray-500 hover:text-gray-900 transition-all duration-200 ${isCollapsed ? "mt-1 mx-auto" : "ml-auto"}`}
            ...
            <Link
              ...
              className={
                location.pathname === item.path 
                  ? `flex items-center ${isCollapsed ? "lg:justify-center lg:gap-0 lg:px-2 lg:py-3" : "gap-3 px-4 py-2.5"} rounded-lg bg-primary-50 text-primary-700 font-medium transition-all duration-200` 
                  : `flex items-center ${isCollapsed ? "lg:justify-center lg:gap-0 lg:px-2 lg:py-3" : "gap-3 px-4 py-2.5"} rounded-lg text-gray-600 hover:bg-primary-50 hover:text-primary-700 transition-all duration-200`
              }
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 border-t border-gray-100 bg-white">
          <button
            ...
            className={`flex items-center ${isCollapsed ? "lg:justify-center lg:gap-0 lg:px-2 lg:py-3" : "gap-3 px-4 py-2.5"} rounded-lg text-gray-600 hover:bg-red-50 hover:text-red-700 transition-all duration-200 w-full text-red-600`}
      ...
      <div className={`flex-1 min-w-0 transition-all duration-300 ${isCollapsed ? "lg:ml-16" : "lg:ml-56"}`}>
